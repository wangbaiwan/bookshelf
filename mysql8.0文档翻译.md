...
### 3.3.2 创建表
创建数据库是简单的，但正如<u><b>SHOW TABLES</b></u>告诉你的那样，此时它是空的：
```shell
mysql> SHOW TABLES;
Empty set (0.00 sec)
```
难的是确定你数据库的结构：你需要哪些表，它们各有哪些列。

你需要一张包含每只宠物记录的表格。这可以称为 **pet** 表，应该最低限度的包含每个宠物的名称。只有名称信息并不有趣，所以表应该有其他信息。例如，你家中多个人有宠物，你可能希望列出每个宠物的主任。你还可能想记录一些宠物的基本信息，如物种和性别。

年龄怎么样？这也许是有意义的，但存储在数据库中并不是一个好主意。年龄会随时间变化，这意味着你需要经常更新这个记录。相反，记录固定值更好，如出生日期。无论何时你需要年龄，你可以通过计算当前日期和出生日期的差值来得到。MySQL提供了进行日期计算的函数，因此这并不困难。储存出生日期来替代年龄还有其他优势：
+ 你可以将数据库用于生成宠物生日提醒的任务（如果你觉得这种查询很愚蠢，请注意，这和你通过一个业务数据库来确认本周或本月需要发送生日祝福，来完成计算机下的私人关怀情景是一样的）
+ 你可以使用当前日期意外的时间来计算时长。例如，你可以在数据库中存储死亡日期，这样你就可以方便地算出宠物死亡时的年龄。

你还能想到其他在 **pet** 表中有用的信息，但目前确认的已经够用：姓名，主任，物种，性别，出生日期，死亡日期。

使用<u><b>CREATE TABLE</b></u>语句来指定表结构：
```shell
mysql> CREATE TABLE pet (name VARCHAR(20), owner VARCHAR(20),
       species VARCHAR(20), sex CHAR(1), birth DATE, death DATE);
```
因为数值长度不等，<u><b>VARCHAR</b></u>类型对姓名，主任和物种列来说是个好选择。这些列的定义不必相同，长度也不必是20.你可以选择1到65535中选择对你来说最合理的值。如果你选择的不好，后续你需要更长的字段，MySQL提供了<u><b>ALTER TABLE</b></u>语句。

有多个值可以代表动物记录中的性别，如 **'m'** 和 **'f'** ，或是可能是 **'male'** 和 **'female'** 。使用 **'m'** 和 **'f'** 最简单。

出生日期和死亡日期使用<u><b>DATE</b></u>类型是明显的选择。

在你创建了表以后，<u><b>SHOW TABLES</b></u>能产生一些输出：
```shell
mysql> SHOW TABLES;
+---------------------+
| Tables in menagerie |
...
+---------------------+
```
要验证表是否按照你预期的方式创建，使用<u><b>DESCRIBE</b></u>语句：
```shell
mysql> DESCRIBE pet;
+---------+-------------+------+-----+---------+-------+
...
+---------+-------------+------+-----+---------+-------+
```
如果你忘了列名称或是列类型，你随时可以使用<u><b>DESCRIBE</b></u>。
